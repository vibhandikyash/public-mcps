...
Three small, focused MCP (Model Context Protocol) servers I built and open-sourced for the community. All are stdio servers, zero-OAuth, easy to install via `npx`.
...
| [`pii-scrubber-mcp`](./pii-scrubber-mcp) | Detect and redact PII (emails, phones, cards, SSNs, API keys, JWTs, private keys, more) with HIPAA / GDPR / PCI policy presets |
...
- **PII scrubber** — every AI workflow eventually pipes user data into a third-party model. This one detects 15 PII categories, ships 5 redaction strategies (including deterministic hashing for join-after-redact), and bundles HIPAA / GDPR / PCI policy presets. Local-only, no network calls.
...
cd pii-scrubber-mcp && npm install && npm test
```
